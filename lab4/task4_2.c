...
#define MSGP "This is message from parent\n"
#define LENP 29
...
int main()
{
	...
	close(fd[0]);
    printf("here\n");
	write(fd[1], MSGP, strlen(MSGP) + 1);
	printf("here\n");
	printf("Message from parent was sent\n"); 
	printf("here\n");
	...
	{
		printf("childrrrrrrrrrrrrrrrrr\n");
		printf("First child process: pid = %d, ppid = %d, pgrp = %d\n", 
		getpid(), getppid(), getpgrp());
		
		char message[LENP] = { 0 };
		printf("Reading messages from parent.\n");
		close(fd[1]);
		if (read(fd[0], message, LENP) < 0)
			printf("No messages from parent.\n");
		else
