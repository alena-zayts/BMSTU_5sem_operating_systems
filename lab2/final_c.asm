...
;в защищ.режиме написали управляющую программу с 0 уровнем привилегий.
;Данная программа выполняет две функции ОС (тк наша программа работает в защищенном режиме с нулевым 
;уровнем привилегий): переход в защищенный режим и выделение сегментов памяти,
;описанных в глобальной таблице дескрипторов и размер, которых мы подсчитываем в программе»



;что пришлось создать в этой программе раз это такоая программа
;мы создаем 2 сист. табл. Табл. дескр. прер. и табл. дескр. сегментов.



;где явно установлен соответсвтующий уровень привилегий
;То, что мы используем системные таблицы.
;привилегии (4 кольциа защиты) != приоритет (приоритет процессов потоков, котоорый назначатся и потом мб пересчитан)


;какаие сегменты вы описали в глобальной таблице и почему (для чего)
;16-разрядный сегмент кода, граница в байтах (G=0)(для реального режима) 
;32-разрядный (D=1) сегмент данных, размер: 4 Гб 	;(для определения объема выделенной памяти)
;32-разрядный (D=1) сегмент кода, граница в байтах (G=0) ;(для защищенного режима)
;32-разрядный (D=1) сегмент данных, граница в байтах (G=0) (от нас-там данные всякие, рязанова хотела 16-р)
...
;iimit=fffff=2^32=4Гб
;32-разрядный (D=1) сегмент данных, граница в блоках по 4 Кбайт (G=1), размер: 4 Гб
;attr_1=92h=10010010b: сегмент данных, разрешены чтение и запись, является сегментом памяти
...
;что написали для исключений - заглушки

;как адресуются аппаратные прерывания в ЗР (ударный)
;Таким образом, обработчики для аппаратных прерываний должны начинаться минимум с 32-го 
;(в винде принято начинать обработку прерываний со смещения 50h, но мы на это забьём, 
;чтобы не пилить ещё дофига лишних обработчиков).

;когда вывызваетсся обаботчик от клавы? таймера?


;какие действия необходимо выполнить для корректного возвращения в РР
; запретить маскрируемые прерывания cli
; сбросить влаг PE (0) в CR0-слово состояния машины
; загрузить в используемые сегментные регистры селекторы соответствующих сегментов
; к регистру CS недопустимо прямое програмнео обращение, поэтому юзаем переход
; ;перепрограммирование ведущего контроллера на 8 обратно
; ;восстанавление масок ведушего и ведомого (из сохраненных)
; ;заугрузить idtr
; ;закрытие линии A20
; разрешить маскрируемые прерывания sti


;что такое теневые регистры, для чего они включены в процесссор, какую информацию содержаь
;линия а20. Что произойдет, если при переходе в ЗР забулем открыть линию а20
;что произойдет, если при возвращении в РР забудем закрыть линию а20