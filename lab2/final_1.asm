...
;сегмент присутсвует в памяти, УП ядра, обычный сегмент, подчиненный сегмент кода, разрешено только исполнение
...
main proc
	...
	;в программе из РФ сброс процессора выполняется засылкой команды FEh в порт 64h контроллера
    ;клавиатуры, в данной - сбросом нулевого бита слова состояния машины, поэтому следующие
	;строки отсутствуют
	;подготовка к возврату из ЗР в РР
    ;MOV AX, 40h
    ;MOV ES, AX
	...
	;(В РФ также осуществляется выборка байта для записи (состояния отключения, 0Fh),
	;но в данной программе выбран другой способ возврата). 
	mov al, 80h
	out 70h, al
